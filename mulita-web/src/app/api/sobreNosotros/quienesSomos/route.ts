...
// GET: obtener los datos de Quienes Somos
...
// PATCH: actualizar Quienes Somos
export async function PATCH(req: NextRequest) {
  ...
  const { data: seccion, error: seccionError } = await supabase
    ...
    .select("id")
    .eq("nombre", "QuienesSomos")
    .single();

  if (seccionError || !seccion) {
    return NextResponse.json(
      { error: "No se pudo obtener id_seccion para 'QuienesSomos'" },
      { status: 500 }
    );
  }
  ...
  if (imagen1_url instanceof File) {
    ...
    const { data, error } = await supabase.storage
      .from("mulita-files")
      .upload(`sobreNosotros/quienes-somos/${Date.now()}_${file.name}`, file);

    if (error) return NextResponse.json({ error: error.message }, { status: 500 });

    imagen1_url = supabase.storage.from("mulita-files").getPublicUrl(data.path).data.publicUrl;
  } else {
    imagen1_url = quienesSomosActual?.imagen1;
  }
  ...
  if (imagen2_url instanceof File) {
    ...
    imagen2_url = quienesSomosActual?.imagen2;
  }
